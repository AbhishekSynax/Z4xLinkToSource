...
WAITING_SUPPORT = set()
ADMIN_REPLY_TRACK = {} # NEW: Button reply ke liye UID track karne ke liye
...
def generate_qr(amount):
    ...
    qr.add_data(f"upi://pay?pa={UPI_ID}&pn=URLSourceZIP&am={amount}&cu=INR")
    qr.make(fit=True)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    "🚀 **Enjoy Premium Experience!**"
        ...
        save_data()
        await context.bot.send_message(uid, msg_text, parse_mode="Markdown")
        await q.edit_message_caption(caption=f"✅ User {uid} Approved/Extended!")
